...
async function listFolder(folderId = ROOT_FOLDER_ID) {
  ...
  await assertDriveItemInRoot(cacheKey);
  const cached = listingCache.get(cacheKey);
  if (cached && cached.expiresAt > Date.now()) return cached.data;

  const folder = await driveRequest(`${DRIVE_API}/files/${encodeURIComponent(cacheKey)}?fields=id,name,mimeType,webViewLink&supportsAllDrives=true`);
  if (folder.mimeType !== FOLDER_MIME) throw new Error('Configured Drive root is not a folder.');
...
async function getDriveItemParents(itemId) {
  return driveRequest(`${DRIVE_API}/files/${encodeURIComponent(itemId)}?fields=id,mimeType,parents&supportsAllDrives=true`);
}

async function isDriveItemInRoot(itemId, seen = new Set()) {
  ...
  const item = await getDriveItemParents(id);
  const parents = Array.isArray(item.parents) ? item.parents : [];
  for (const parentId of parents) {
    if (await isDriveItemInRoot(parentId, seen)) {
      ancestryCache.set(id, true);
      return true;
    }
  }

  ancestryCache.set(id, false);
  return false;
}

async function assertDriveItemInRoot(itemId) {
  if (await isDriveItemInRoot(itemId)) return true;
  throw new Error('Drive item is outside the configured resource root.');
}
